...
test('readiness validator requires contiguous migrations and commercial assets',()=>{assert.match(ready,/migration_sequence/);assert.match(ready,/web\/verify\.html/);assert.match(ready,/web\/developers\.html/);assert.match(ready,/expected_at_least_17/);});
test('production smoke covers commercial and trust surfaces',()=>{for(const marker of ['/api/v1/security/capabilities','/api/v1/payments/capabilities','/api/v1/validation/capabilities','/api/v1/reputation/capabilities','/api/v1/developer/capabilities','/api/v1/launch/capabilities'])assert.match(smoke,new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));});
