...
test('DNS verification produces evidence but cannot mint passed result',()=>{assert.match(dns,/next_step:'A qualified validator must sign the final passed result using this exact evidence_digest.'/);assert.doesNotMatch(dns,/outcome='passed'/);});
...
test('validation stats are aggregate and privacy bounded',()=>{assert.match(dns,/\/api\/v1\/validation\/stats/);assert.match(dns,/Aggregate counts only/);assert.doesNotMatch(dns,/SELECT \*/);});
