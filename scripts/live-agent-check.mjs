...
const evidence = {
  event: "external-agent-production-check",
  source: "github-actions",
  run: process.env.GITHUB_RUN_ID || `manual-${Date.now()}`
};
...
const a2a = await requestJson("/a2a", {
  method: "POST",
  headers: { "content-type": "application/json", "A2A-Version": "1.0" },
  body: JSON.stringify({
    jsonrpc: "2.0",
    id: "external-a2a-check",
    method: "SendMessage",
    params: {
      message: {
        role: "ROLE_USER",
        ...
        parts: [{
          data: { action: "verify_proof", arguments: { proof_id: create.proof_id, data: evidence } },
...
const mcpVerified = await mcp("tools/call", {
  name: "accord_trace_verify",
  arguments: { proof_id: create.proof_id, data: evidence }
}, "external-mcp-verify");
